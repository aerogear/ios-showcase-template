...
class StorageViewController: UITableViewController {
    static let storageStoryBoard = UIStoryboard(name: "storage", bundle: nil)
    
    @IBOutlet weak var addButton: UIBarButtonItem!
    var notes = [Note]()
    
    let kcWrapper: KeychainWrapper = KeychainWrapper.standard
    ...
    */
    ...
    func readNote(identifier: Int, onComplete: @escaping (Error?, Note?) -> Void) {
        self.storageService?.read(identifier: identifier) {
            error, note in
            if(note != nil) {
                onComplete(nil, note!)
            } else {
    ...
    static func loadViewController() -> UIViewController {
        return storageStoryBoard.instantiateViewController(withIdentifier: "StorageViewController")
    }
